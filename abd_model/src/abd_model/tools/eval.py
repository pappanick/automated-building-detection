...
from abd_model.core import load_config, load_module, check_model, check_channels, check_classes
...
def add_parser(subparser, formatter_class):
    parser = subparser.add_parser(
        "eval", help="Evals a model on a dataset", formatter_class=formatter_class)
    parser.add_argument(
    ...
    data.add_argument("--tiles_weights", type=str,
                      help="path to csv tiles cover file, with specials weights on [optional]")
    ...
    ev.add_argument("--metrics", type=str, nargs="+",
                    help="metric name (e.g QoD IoU MCC)")
    ev.add_argument("--checkpoint", type=str, required=True,
                    help="path to model checkpoint.")
    ev.add_argument("--workers", type=int,
...
def main(args):
    ...
    print("abd eval on 1 GPU, with {} workers, and {} tiles/batch".format(args.workers, args.bs))
    ...
    for channel in config["channels"]:
        for band in channel["bands"]:
            print("Channel {}:\t\t {} - (band:{})".format(num_channel,
                  channel["name"], band))
            num_channel += 1
            ...
            for c, classe in enumerate(config["classes"]):
                if classe["weight"] != 0.0 and classe["color"] != "transparent":
                    for k, v in metrics.get()[c].items():
                        print("{}{:.3f}\t {:.3f}".format(
                            (classe["title"] + " " + k).ljust(25, " "), v["μ"], v["σ"]))
